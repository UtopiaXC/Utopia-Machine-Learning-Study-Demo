...
# Configs
# Radius of each point to find others point near it.
epsilon = 10
min_points = 4
data_generator_set_count = 4
data_generator_data_for_each_count_count = 30
...
# Make starting label be 1.
label = 1

# Traverse every element in the data set and find all points near each point, and loop.
for index in range(len(data_set)):
    # Skip points that is clustered.
    ...
    # If each point in the cluster do not have any neighbors in the radius, break the loop.
    while len(cluster_indexes) != cluster_count:
        ...
        for cluster_index in cluster_indexes:
            for data_set_index in range(len(data_set)):
                ...
                # Find and storage every point that in the radius area and let them be a cluster.
                if distance <= epsilon:
                    cluster_indexes.append(data_set_index)
                    data_set[data_set_index][2] = label
    # If count of points in the cluster is less than min limit, let them be noise.
    if len(cluster_indexes) < min_points:
        for index_reset in cluster_indexes:
            data_set[index_reset][2] = 0
        continue
    # This cluster is formed, make new label.
    label += 1

# Process data to generate plot image.
